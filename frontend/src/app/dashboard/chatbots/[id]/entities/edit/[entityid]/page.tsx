...
import { createEntity, getEntity, updateEntity } from "@/services/entities_service";
...
export default function CreateEntityPage() {
  ...
  const serviceid = user?.services[0].id; 
   ...
   useEffect(() => {
     ...
     const fetchEntity = async () => {
       try {
        const entity:Entity = await getEntity(user.services[0].id, params.id as string, params.entityid as string);
        ...
        if (entity.entity_type == 'list') {
          ...
          for (const key in entity.value) {
            entityValues.push({
              value: key,
              synonyms: entity.value[key]
            })
          }
          setEntityValues(entityValues)
        } else if (entity.entity_type == 'pattern') {
          setPatternValue(entity.value.regex[0])
        }
        
       } catch (err) {
         console.log("No entities found or failed to load", err);
       }
     };
 
     fetchEntity();
   }, [user, params.id, params.entityid]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateEntity(serviceid, chatbot_uuid, entityid, data)
