...
useEffect(() => {
  ...
  const fetchIntent = async () => {
    ...
    try {
    const intent: Intent = await getIntent(user?.services[0].id, params.id as string, intentid);
    setName(intent.name)
    setDescription(intent.description || '')
    setActionName(intent.action.name)
    setTrainingPhrases(intent.phrases)
    setResponses(intent.default_intent_responses)
    setWebhookEnabled(intent.action.webhook)
    setParameter(intent.action.parameters.map(i => ({ parameter: i.parameter, required: i.required, messages: i.message ? [i.message] : [] })))
 
    } catch (err) {
      console.log("No Intent found or failed to load", err);
    }
  };
...
const intentid = params.intentid as string;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateIntent(serviceid, chatbot_uuid, intentid, data)
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(e) => {setWebhookEnabled(!webhookEnabled)}}
                        />
                          Enable Webhook
                        </Label>
