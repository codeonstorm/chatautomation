...
def create_domain(
    service_id: int, domain: DomainCreate, session: Session = Depends(get_session)
):
...
def read_domain(
    service_id: int, domain_id: UUID, session: Session = Depends(get_session)
):
    domain = session.exec(
        select(Domain).where(Domain.service_id == service_id, Domain.uuid == domain_id)
    ).first()
    if not domain:
...
def delete_domain(
    ...
    session.commit()
    return ResponseSchema(success=True, message="Domain deleted successfully")
