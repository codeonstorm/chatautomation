...
def get_datasets(
  *,
  ...
  if not plans:
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No datasets found.")
  return plans
...
def update_dataset(
  *,
  ...
  dataset = session.exec(
    select(Dataset).where(Dataset.service_id == service_id, Dataset.id == dataset_id)
  ).first()
